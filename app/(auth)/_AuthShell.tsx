    ...
    <main className="grid min-h-screen grid-cols-1 md:grid-cols-[1.05fr_1fr]">
      ...
      <div className="flex items-center justify-center bg-kola-cream-light p-10">
        <div className="w-full max-w-[360px]">
          <h2 className="mb-1.5 text-2xl font-extrabold tracking-tight">{titre}</h2>
          <p className="mb-7 text-sm text-kola-muted">{sousTitre}</p>
          {children}
        </div>
      </div>
    </main>
...
export function ChampTexte({
  ...
  return (
    <label className="mb-4 block">
      ...
      <input
        {...props}
        className="w-full rounded-xl border border-kola-border-dark bg-white px-3.5 py-3 text-sm text-kola-text outline-none focus:border-kola-accent focus:ring-4 focus:ring-kola-accent/15"
      />
    </label>
  );
}
